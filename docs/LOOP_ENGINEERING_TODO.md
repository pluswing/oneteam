...
- [x] **Done**: 共通AsyncStateで主要一覧・detail・diff・Settingsのloading / retrying / empty / errorをARIA付きで統一し、明示的な再試行操作を提供する。Issue / PR / Agent runは通信失敗時も直前データを維持し、diffは同一revisionのpatchだけを保持する。未読込Settingsの編集を防止し、Human / Provider waiting calloutも共通化する
- [x] **Done**: 全画面共通のfocus-visible ring、skip link、route変更時のmain focus、reduced-motion、project tools menuのArrow / Home / End / Escape操作、diffの`j` / `k` shortcutとfile list roving focus、ARIA current / pressedを実装する。PR subtabは`tablist` / roving tab stopとArrow Left / Right・Home / End操作を持つ。Project pickerからIssues / PR / Agent runs / Repository / Settings / Loops、日本語PR diffまで、keyboard経路とcomputed colorによるWCAG contrastをChromium E2Eで画面別監査する
