...
- [ ] **Partial**: 全画面共通のfocus-visible ring、skip link、route変更時のmain focus、reduced-motion、project tools menuのArrow / Home / End / Escape操作、diffの`j` / `k` shortcutとfile list roving focus、ARIA current / pressed、keyboard E2Eは実装済み。画面別のkeyboard操作とcontrastの網羅監査は未完了
